...
namespace kv_intervall_maximum_filter {
  ...
  private:
    ...
    void jarnik_prim(const algen::WEdgeList& edge_list,
      ...
      // initialise priority queue
      // This should be reasonably fast since the siftup of each push will immediately notice that it is already on the correct position
      PriorityQueue q;
      q.push({0, 0});
      for (long i = 1; i < num_vertices; i++) {
        q.push({algen::WEIGHT_MAX, i});
  ...
  class IntervallMaximumFilterPH {
